...
public:
	...
	t_weight optimize(std::vector<bool>& solution, double timeout_sec) {
		...
		for (int id = softVarFalsified.size() - 1; id >= 0; id--) {
			...
			if (solver->solveLimited(assumSatisfied,
						timeout_sec / (double)(softVarFalsified.size() + 1))
					!= 1) {
...
public:
	...
	struct LitCard {
		...
		LitCard(std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>> card, int atMost,
				t_weight initialWeight)
...
public:
	...
	void setCoef(double initialCoef, double coefOnRefTime) {
		...
		_base = (-initialCoef
				/ (coefOnRefTime
						* W((-initialCoef * std::exp(-initialCoef / coefOnRefTime)) / coefOnRefTime)));
	}
...
public:
	void printInfo() {
		...
		std::cout << "c Stop unsat improving after " << totalSolveTimeout.getVal() << " sec"
				  << std::endl;
	...
	int addClause(const std::vector<int>& clause, std::optional<long long> w = {}) {
		if (w.has_value() == false) { // Hard clause
			...
			if (solutionCost != std::numeric_limits<t_weight>::max()) {
				if (!is_incremental) {
					std::cerr << "c Error: You cannot add a soft clause after the first solve if the incremental mode is not activated."
							  << std::endl;
					exit(-1);
				}
			}
...
private:
	std::set<int> initAssum(t_weight minWeightToConsider = 1) {
		...
		for (auto itOnMapWeight2Assum = _mapWeight2Assum.rbegin();
				itOnMapWeight2Assum != _mapWeight2Assum.rend(); ++itOnMapWeight2Assum) {
...
public:
	...
	void setIncremental(bool value = true) { is_incremental = value; }
		...
		for (;;) {
			...
			if (resultLastSolve) { // SAT
					...
					if (resultLastSolve == 0) { // UNSAT
						...
						if (_multiSolveStrategy && (conflict.size() > 3)) {
							...
							while (conflict.size() > 3
								...
								auto p = oneMinimize(tmp, maxLastSolveOr1 * getTimeRefCoef() * 0.1,
										10, true);
								...
								auto res = solver->solveWithTimeout(vAssum,
										maxLastSolveOr1 * getTimeRefCoef() - C.tacSec());
								if (res == -1) {
				...
				for (auto c : _cardToAdd) {
					std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>> card =
							std::make_shared<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>>(this,
									std::get<0>(c), 1);
					int newAssumForCard = card->atMost(1);
					if (newAssumForCard != 0) {
						assert(_poids[newAssumForCard] == 0);
...
public:
	...
	double oneMinimize(std::vector<int>& conflict, double referenceTimeSec, unsigned int B = 10,
			bool sort = true) {
		Chrono C;
		double minimize = 0;
		double noMinimize = 0;
	...
	void reduceCliqueV2(std::list<int>& clique) {
		...
		for (auto posImpliquant = clique.begin(); posImpliquant != clique.end(); ++posImpliquant) {
			...
			for (++posImpliquant2; posImpliquant2 != clique.end();) {
				if (solver->solveLimited(std::vector<int>({-(*posImpliquant), -(*posImpliquant2)}),
							10000)
						!= 0) { // solve != UNSAT
	...
	void addWeight(int lit, long long weight) {
		...
		if (_poids[lit] == 0) {
			...
			if (_poids[lit] > 0) {
				...
				assert(_poids[lit] < 0
								? !_mapAssum2Card[abs(lit)].has_value()
								: true); // If -lit becomes a soft var, it should not be a cardinality
		...
		if (_mapAssum2Card[var].has_value()) { // If there is a cardinality constraint associated to this soft var
			...
			if (forCard != 0) {
				if (_mapAssum2Card[abs(forCard)].has_value() == false) {
					_mapAssum2Card[abs(forCard)] = LitCard(_mapAssum2Card[var]->card,
							_mapAssum2Card[var]->atMost + 1, _mapAssum2Card[var]->initialWeight);
